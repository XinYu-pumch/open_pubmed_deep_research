...
class TableProcessor(BaseProcessor):
    ...
    def assign_ocr_lines(self, ocr_blocks: list):
        ...
        # Check if recognition model is a mock (for compatibility with bypass)
        from unittest.mock import Mock
        if isinstance(self.recognition_model, Mock):
            # Skip OCR processing for mock recognition model
            print("Warning: Using mock recognition model, skipping table OCR processing")
            for block in ocr_blocks:
                block["table_text_lines"] = []  # Empty text lines for tables
            return
